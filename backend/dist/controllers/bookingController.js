...
exports.bookingCheckout = exports.createBooking = void 0;
...
const createBooking = async (req, res) => {
    ...
    `;
        const activeSpace = await db_1.default.query(activeQuery, [userIdInt]);
        if (activeSpace.rows.length > 0) {
            return res.status(403).json({
                success: false,
                message: "You already have an active or upcoming booking. Please cancel or checkout first.",
    ...
    `;
        const conflict = await db_1.default.query(checkQuery, [
            deskIdInt,
            date,
            start_time,
            end_time,
      ...
      RETURNING id
    `;
        const newBooking = await db_1.default.query(insertQuery, [
            userIdInt,
            deskIdInt,
            date,
            start_time,
...
const bookingCheckout = async (req, res) => {
    ...
    try {
      ...
      AND status = 'CONFIRMED'
    `;
        const check = await db_1.default.query(checkQuery, [userIdInt, deskIdInt]);
      ...
      AND status = 'CONFIRMED'
    `;
        await db_1.default.query(toggleQuery, [deskIdInt, userIdInt]);
        res.status(200).json({
            success: true,
...
exports.bookingCheckout = bookingCheckout;
